...
Spot.prototype.flushOptions= function() {
    ...
    if (this.spotClass === 'img-spot') {
        var $edImg1 = this.$edSpot.find('img').eq(0),
            $edImg2 = this.$edSpot.find('img').eq(1),
            $prevImg1 = this.$prevSpot.find('img').eq(0),
            $prevImg2 = this.$prevSpot.find('img').eq(1);

        this.$prevSpot.find('img').removeAttr('style');

        //add image width & height to fix bug in IE10 with cached image
        //as IE10 will not be able to get image dimensions & will not fire
        //image load event & image.complete will be false when hotspotter run
        if ($edImg1.width()) {
            $prevImg1.attr('width', $edImg1.width());
            $prevImg1.attr('height', $edImg1.height());
        }

        if ($edImg2.width()) {
            $prevImg2.attr('width', $edImg2.width());
            $prevImg2.attr('height', $edImg2.height());
        }
    }
};
...
var Editor = {
    ...
    isPreview: false,
    $edDiv: null, //editing div
    $prevDiv: null,//preview div
    $edImg: null, //image dom in editor
    $prevImg: null, //image dom in preview
    curSpot: null, //current spot object that's being edited
    optionBar: {}, //option bar dom elements
    ...
    togglePreview: function () {
        if (this.inPreview) {
            ...
            var winW  = $(window).width(),
                ...
                left = 0,
                top = 45;  //45 to account for end preview btn

            if ( winW > prevW ) {
                //position preview image in center horizontally
                left = (winW - prevW) / 2;
            }

            //2*45 to ensure image wont cover end preview btn
            if ( winH > (prevH + 2 * 45) ) {
                //position preview image in center vertically
                top = (winH - prevH) / 2;
            }

            this.$prevDiv.parent().css('left', left);
            this.$prevDiv.parent().css('top', top);
    ...
    loadCode: function () {
        ...
        $hsArea.find('.hs-wrap')
               .each(function () {
                   ...
                   spotObj.options.spType = spHandler || 'General';
                   spotObj.options.spAnim = !!spHdOpts;
    ...
    _updateOptions: function () {
        ...
        for (var i in this.optionBar) {
            ...
            if (i === 'spAnim') {
                this.optionBar['spAnim'].prop('checked', options['spAnim']);
        ...
        for (var i in this.optionBar) {
            if (i === 'spBradius') {
                if (this.isEnabled('spBradius')) {
                    this.curSpot.dim[2] = parseInt(this.optionBar['spBradius'].val(), 10);
                }
                continue;
            }
            ...
            if (i === 'spAnim') {
                options[i] = this.optionBar[i].is(':checked');
